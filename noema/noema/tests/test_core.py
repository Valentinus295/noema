...
from noema.core.settings import (
    Settings, BrokerConfig, RiskConfig, TradingConfig, NIMConfig,
    load_settings,
)
...
class TestNoemaConfig:
    ...
    def test_default_config(self):
        ...
        assert config.risk.risk_pct_per_trade == 0.25
        assert config.risk.daily_loss_limit_pct == 1.0
        assert config.risk.max_concurrent_positions == 3
        assert config.risk.max_open_trades == 5  # backward-compat field
    ...
    def test_default_risk_config(self):
        ...
        config = RiskConfig()
        assert config.risk_pct_per_trade == 0.25
        assert config.risk_per_trade == 0.01  # backward-compat
        assert config.max_concurrent_positions == 3
        assert config.max_open_trades == 5  # backward-compat
        assert config.min_risk_reward == 2.0  # backward-compat
    ...
    def test_custom_config_overrides(self):
        ...
        assert config.risk.risk_pct_per_trade == 0.02
        assert config.risk.max_concurrent_positions == 2

    def test_load_config_from_yaml(self, tmp_path):
        """load_settings should read from a YAML file."""
        yaml_content = {
            "broker": {"type": "mt5", "mt5_login": 99999},
            "risk": {"risk_pct_per_trade": 0.02, "risk_per_trade": 0.02,
                     "max_open_trades": 4, "min_risk_reward": 3.0},
    ...
    def test_load_config_non_existent(self):
        """load_settings should return defaults if file doesn't exist."""
        config = load_settings("/nonexistent/path.yaml")
        assert config.broker.type == "paper"
