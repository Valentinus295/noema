...
Uses NIM + instructor to reflect on trade outcomes and store lessons learned.
This is where the system gets smarter over time.

Updated: Uses LearningOutput with instructor guaranteed valid outputs.
Tracks patterns across trades for institutional memory.
"""
...
class LearningAgent(LLMAgent):
    ...
    Uses instructor for guaranteed valid outputs with pattern tracking.
    """
    ...
    response_model = TradeReflection  # Legacy model
    structured_model = LearningOutput  # New instructor-powered model
    temperature = 0.4
    tier_name = "fast"
    ...
    def _build_user_message(self, context: dict[str, Any]) -> str:
        ...
        # Include past reflections for pattern comparison
        past_reflections = self.memory.get_relevant_reflections(symbol, limit=3)
        if past_reflections:
            parts.append(f"\n## Past Reflections for {symbol} ({len(past_reflections)} stored):")
            for i, ref in enumerate(past_reflections):
                parts.append(f"  {i+1}. Pattern: {ref.get('pattern_type', 'unknown')} | "
                           f"Lesson: {ref.get('lesson_learned', 'N/A')[:80]}")

        parts.append("\n## Requirements:")
        parts.append("- Identify the pattern type (trend_continuation, reversal, breakout, etc.)")
        parts.append("- List specific, actionable lessons (minimum 1)")
        parts.append("- Suggest parameter adjustments if any system rules should change")
        parts.append("- Compare with past similar patterns if available")
        parts.append("- Decide: should we repeat this pattern type?")

        return "\n".join(parts)

    def _to_report(self, result: Any, llm_latency_ms: float) -> AgentReport:
        """Convert LearningOutput or TradeReflection to AgentReport and store in memory."""
        # Handle new LearningOutput model
        if isinstance(result, LearningOutput):
            # Store reflection with pattern tracking
            reflection_data = result.model_dump()
            reflection_data["symbol"] = self.memory.get_context("symbol", "UNKNOWN")
            self.memory.store_reflection(reflection_data)

            adj_summary = ""
            if result.parameter_adjustments:
                adjs = [f"{a.parameter_path}→{a.suggested_value}" for a in result.parameter_adjustments[:3]]
                adj_summary = f" | Adjustments: {'; '.join(adjs)}"

            return AgentReport(
                agent_name=self.name,
                signal="COMPLETE",
                confidence=result.repeat_confidence if result.should_repeat else 0.3,
                data=result.model_dump(),
                reasoning=(
                    f"Pattern: {result.pattern_type.value} | "
                    f"Lessons: {'; '.join(result.lessons[:2])} | "
                    f"Should repeat: {result.should_repeat} "
                    f"(sim: {result.similar_trades_count})"
                    f"{adj_summary}"
                ),
                agent_type=AgentType.LLM,
                llm_latency_ms=llm_latency_ms,
            )
        # Handle legacy TradeReflection
